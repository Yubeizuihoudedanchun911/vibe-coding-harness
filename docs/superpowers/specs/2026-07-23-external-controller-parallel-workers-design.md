...
shell。只有 Controller 在运行创建时解析、校验并冻结 `CommandSpec`；非空项目
命令目录还必须由用户通过 `--allow-project-commands` 显式授权。Planner、
Worker 和 Evaluator 只能从冻结目录中选择稳定 command ID，不能返回或修改
`argv`、`cwd`、timeout、环境或新的命令对象。Planner 可以追加目录内的任务
检查 ID，不能删除、替换或放宽项目/CLI 冻结的强制门禁。状态和 Artifact
不持久化 Provider secret。
...
- Worker 只在声明范围内留下 raw working-tree edits 和结构化 handoff；
- Worker 不 stage、commit、amend、rebase 或移动任何 Git ref；
- Controller 独立审计 raw delta、rename 两端、删除、gitlink 和
  `.gitmodules`；
- Controller 通过临时 index 创建一个确定性 non-merge source commit；
- Controller 先持久化 prepared source CAS，再只推进保留的 task ref。

Controller 使用
NUL-delimited status/diff 获取实际修改，不信任 Worker 自述。Codex
`workspace-write` 只授权产品文件写入，不授权 Git metadata；这也是 source
commit 必须由 Controller 创建的原因。
  ...
  "tasks": [
    {
      ...
      "exclusive_resources": [],
      "acceptance_checks": ["scheduler-tests"],
      "max_attempts": 3
    }
  ]
}
...
- `worker_type` 来自注册表；
- path scope 合法，所有 command ID 均来自冻结且已授权的目录；
...
only supplied checks, and leave scoped working-tree edits for the Controller
...
Worker 输出包含 task/attempt identity、`COMPLETED | BLOCKED`、changed paths、
checks、residual risks 和 blocker。Controller 从 raw worktree delta、临时 index
和进程结果独立重建关键事实，不信任 Agent 自报路径或测试结论。
...
1. 验证 detached worktree、base、raw delta、protected Git 和
   path/resource contract；
2. 通过临时 index 创建并审计唯一的 Controller-owned source commit；
3. 从当前 integration head 创建临时 candidate branch/worktree；
4. cherry-pick 已审计的单个 source commit；
5. 拒绝冲突、merge commit、越界路径和 control path 修改；
6. 在 candidate 上重跑 task checks 和不可删除的项目门禁；
7. 冻结与 candidate SHA 绑定的 verification Artifact；
8. 准备 `pending_integration` 并通过 CAS 推进 run ref。
